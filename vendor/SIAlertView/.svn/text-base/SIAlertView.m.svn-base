...
//    appearance.buttonFont = [UIFont systemFontOfSize:[UIFont buttonFontSize]];
    ...
    appearance.contentPaddingLeft = 10;
...
- (void)validateLayout
{
	...
	if (self.titleLabel) {
        ...
        self.titleLabel.frame = CGRectMake(_contentPaddingLeft, y, self.containerView.bounds.size.width - _contentPaddingLeft * 2, height);
        y += height;
	}
    if (self.messageLabel) {
        ...
        self.messageLabel.frame = CGRectMake(_contentPaddingLeft, y, self.containerView.bounds.size.width - _contentPaddingLeft * 2, height);
        y += height;
    }
    if (self.items.count > 0) {
        ...
        if (self.items.count == 2) {
            CGFloat width = (self.containerView.bounds.size.width - _contentPaddingLeft * 2 - GAP) * 0.5;
            UIButton *button = self.buttons[0];
            button.frame = CGRectMake(_contentPaddingLeft, y, width, BUTTON_HEIGHT);
            button = self.buttons[1];
            button.frame = CGRectMake(_contentPaddingLeft + width + GAP, y, width, BUTTON_HEIGHT);
        } else {
            for (NSUInteger i = 0; i < self.buttons.count; i++) {
                UIButton *button = self.buttons[i];
                button.frame = CGRectMake(_contentPaddingLeft, y, self.containerView.bounds.size.width - _contentPaddingLeft * 2, BUTTON_HEIGHT);
...
#endif
                                actualFontSize:nil
                                      forWidth:CONTAINER_WIDTH - _contentPaddingLeft * 2
...
- (CGFloat)heightForMessageLabel
{
    ...
    if (self.messageLabel) {
        ...
        CGSize size = [self.message sizeWithFont:self.messageLabel.font
                               constrainedToSize:CGSizeMake(CONTAINER_WIDTH - _contentPaddingLeft * 2, maxHeight)
...
- (void)setContentPaddingLeft:(CGFloat)contentPaddingLeft
{
    if (_contentPaddingLeft == contentPaddingLeft) {
        return;
    }
    _contentPaddingLeft = contentPaddingLeft;
}
